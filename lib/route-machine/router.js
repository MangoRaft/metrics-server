...
const RPS_SWEEPER = 1000// Requests rate sweeper
const START_SWEEPER = 30000// Timer to publish router.start for refreshing state
const CHECK_SWEEPER = 30000// Check time for watching health of registered droplet
const MAX_AGE_STALE = 120000// Max stale age, unregistered if older then 2 minutes
/**
 *
 *
 */
...
var Router = module.exports = function(instance, options) {
	...
	this.log = new log.Logger(raft.config.logs.log, this.uid, (cluster.isMaster ? 'head' : 'slave'), 'system', 'router')

	this.log.write('Router startting... ')
	this.log.write('Process type is ' + (cluster.isMaster ? 'Master' : 'Worker'))
};
//
...
Router.prototype.start = function() {
	...
	if (cluster.isMaster) {
		...
		this.setup_sweepers()
		this.server.listen(raft.config.router.http.port)
...
Router.prototype.register_droplet = function(url, host, port, tags, app_id) {
	...
	this.log.write("Registering " + url + " at " + host + ":" + port + "")
	this.log.write("" + droplets.length + " servers available for " + url + "")
}

Router.prototype.unregister_droplet = function(url, host, port) {
	this.log.write("Unregistering " + url + " for host " + host + ":" + port)
	var droplets = this.droplets[url] || []
	for (var i = 0; i < droplets.length; i++) {
		var d = droplets[i]
